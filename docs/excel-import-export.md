...
Vercel serves the React build from `dist` and routes `/api/excel-master/*` to the serverless Elysia handler in `api/[...path].ts`. The current repository adapter is in-memory, so imported data resets when the serverless instance is replaced; connect a durable adapter before using this in production.
